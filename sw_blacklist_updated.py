...
def primary_blacklist(resamp_pan, sorted_node_list_idx):
    t1 = time.time()
    blacklist = []
    sorted_node_arr = np.array(sorted_node_list_idx)
    
    for idx, node in enumerate(sorted_node_list_idx):
        walker, cyc = node[1] , node[0]
        merged_walkers_list = []
        cyc_resamp_pan = resamp_pan[cyc][0]
        idx_cyc = np.where(sorted_node_arr[:,0] == cyc-1)[0]
        walkers_at_cyc = sorted_node_arr[:,1][idx_cyc]
        for idx2, item in enumerate(cyc_resamp_pan):
            if walker in item[1] and item[0] == 3:
                merged_walkers_list.append(idx2)
        for merged_walker in merged_walkers_list:
            if merged_walker not in walkers_at_cyc and node not in blacklist:
                blacklist.append(node)
    t2 = time.time()
    print(f'Time taken to compute the blacklist for this tree: {t2-t1}')
    return(blacklist)
...
def get_accepted_sw(h5_path, run_idx, window_length, wt_path):
    ...
    for index in range(len(eff_subtrees_list)):
        ...
        blacklist = primary_blacklist(resamp_pan, sorted_node_list[index])

        print(f'Length of the blacklist: {len(blacklist)}')
...
pkl.dump(accepted_sliding_windows,  open(f'rw_tld_run{run_idx}_lagtime{window_length}.pkl','wb'))

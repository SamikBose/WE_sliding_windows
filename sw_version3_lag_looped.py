...
def match_weights(eff_subtree, root_node, window_length, all_wts):
    SIGNAL = True
...
def check_out_of_tree_merge(all_node_dict, merging_dict, dict_walkers_each_cycle):
    ...
    for idx, keys in enumerate(all_node_dict.keys()):
        if idx > 0:
            ...
            #ipdb.set_trace()
            if child_walker in merging_dict[child_cycle - 1].keys(): #if the child is present
                #print(f'child walker:  {child_walker}')
                ...
                for node in merged_nodes:
                    if node not in dict_walkers_each_cycle[child_cycle]:
                        #print(f'Full Dict: {all_node_dict}, Cycle:{child_cycle}, keep_merge_walker: {child_walker}, out of tree walker {node}...')
                        SIGNAL = False
...
def get_accepted_sw(h5_path, window_length, wt_path, n_cycs, n_walkers ):

    t = time.time()
    all_wts_all_run = pkl.load(open(wt_path,'rb'))
    accepted_sw = {}
    wepy_h5 =  WepyHDF5(h5_path, mode='r')
    ...
    for run in range(n_runs):
        accepted_sw[run] = []
        all_subtrees, resamp_pan = gen_all_trees(h5_path,run)
        eff_subtrees_list, sorted_node_list = effective_subtrees(all_subtrees, window_length)

        all_wts = all_wts_all_run[run]
        print(f'\nRun{run}, number of effective subtrees: {len(eff_subtrees_list)}')

        merging_dict = merged_dict(resamp_pan)

        for index in range(len(eff_subtrees_list)):
            
            print(f'\nRun{run}, walker_tree_index: {index}, number of nodes: {len(sorted_node_list[index])}')
            t1 = time.time()
            true_count = 0
            ...
            for each_sorted_node in sorted_node_list[index]:
                ...
                if current_cycle <= final_cycle_in_subtree - window_length:
                    signal, init_wt, final_wt, all_node_dict, tl_nodes_list = match_weights(eff_subtrees_list[index], each_sorted_node, window_length, all_wts)
                    if signal == True:
                        ...
                        if signal_merging == True:
                            ...
                            for tl_node in tl_nodes_list:
                                accepted_sw[run].append([(each_sorted_node[1], each_sorted_node[0]), (tl_node[1],tl_node[0])])
            ...
            print(f'Accepted node, false_counts_wt, false_count_merging: {true_count}, {false_count_wt}, {false_count_merging}')
        #print(merging_dict)
    return(accepted_sw)


## Main code ###
lag_list = [2,3,4,5,15,20,25,30,40,50,75,100,125,150,200]
#lag_list = [1, 5, 10]

for window_length in lag_list:
    t_start = time.time()
    n_cycs = int(sys.argv[1])
    n_walkers = int(sys.argv[2])

    base_path = '/dickson/s1/bosesami/time_lagged_data_WE/'
    h5_path = f'{base_path}/clone_RW_{n_walkers}_{n_cycs}.h5'
    wt_path = f'{base_path}/randomwalk_sims/all_run_wts/wts_allruns_{n_walkers}_{n_cycs}.pkl'
    out_path = f'{base_path}/TLD_{n_cycs}_{n_walkers}/rw_tld_all_runs_dict_lagtime{window_length}.pkl'

    print(f'\nRunning for window length: {window_length}')

    accepted_sliding_windows = get_accepted_sw(h5_path, window_length, wt_path, n_cycs, n_walkers)
    pkl.dump(accepted_sliding_windows,  open(out_path,'wb'))

    for keys in accepted_sliding_windows.keys():
        print(f'Run idx: {keys}, lag{window_length}, length of accepted tld:{len(accepted_sliding_windows[keys])}')

    t_end = time.time()
    print(f'Time taken for lagtime {window_length}, {t_end-t_start} seconds...')
